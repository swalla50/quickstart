...
function AssignGroupsModal(props) {
    const [user, setUser] = useState([]);


    var match = props.Group.MyUserId;
    const [groupListAvailable, setgroupListAvailable] = useState([]);
    ...
    useEffect(() => {
        ...
        axios.get(`getusergrouplist/getGroups`)
            .then((response) => {
                setgroupListAvailable(response.data);
                console.log('USERS GROUPS', (response.data.filter(item => item.MyUserId == props.Group.myUserId)));
    ...
    function assignRight() {
        if (checked.length > 1) {
            for (var i = 0; i < checked.length; i++) {
                ...
                axios.put('GetGroupUsers/assignUserGroups', newUserAssigned)
                    .then(res => {
                        // getavailableUser();
    ...
    console.log("FINAL USER LIST", groupListAvailable)
    return (
        <div className='AssignModuleRights'>
            ...
            >
                ...
                <Modal.Header closeButton>
                    Update Groups For User : {props.Group.FullName}
                </Modal.Header>
                <Modal.Body>
                        ...
                        <div className='upper'>
                            <div className='rights-available'>
                                ...
                                <div className='available-module-container'>
                                    {groupListAvailable.filter(item => item.MyUserId === match && item.inGroup == false).map(item => (
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <ListItemButton className='module-name' role={undefined} onClick={handleToggle(item)} dense>
                                                            ...
                                                            <ListItemText id={labelId} primary={`${'(' + item.groupId + ')' + ' ' + item.vendorName}`} />
                                                        </ListItemButton>
                                                    </ListItem>
                                                );
                                            })}
                            ...
                            <div className='rights-available'>
                                ...
                                <div className='assign-unassign-right-btn'>
                                    <Button onClick={()=>{assignRight();getGroupUsers()}} style={{ background: 'gray', border: 'none' }} className='delete-right'>Assign  <FontAwesomeIcon className="bk-coin-icon" icon={faChevronRight} size='1x' /><FontAwesomeIcon className="bk-coin-icon" icon={faChevronRight} size='1x' /></Button>
                                    <Button onClick={()=>{unassignRight();getGroupUsers()}} style={{ background: 'gray', border: 'none' }} className='delete-right'> <FontAwesomeIcon className="bk-coin-icon" icon={faChevronLeft} size='1x' /><FontAwesomeIcon className="bk-coin-icon" icon={faChevronLeft} size='1x' /> Un-Assign</Button>
                                </div>
                            ...
                            <div className='rights-assigned'>
                                ...
                                <div className='available-module-container'>
                                    {groupListAvailable.filter(item => item.MyUserId == props.Group.MyUserId && item.inGroup == true).map((item) => (
                                        <List sx={{ width: '100%', maxWidth: 360, bgcolor: 'background.paper' }}>
                                            {[0].map((value) => {
                                                const labelId = `checkbox-list-label-${item}`;
