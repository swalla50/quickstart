...
function ClientInventoryChart(props) {
        ...
        setTimeout(() => {
            ...
            axios.get(`getinventory/getInventoryList`)
                .then((res) => {
                    ...
                    setData(res.data.filter(inv => inv.Vendor == props.Vendor))
                    // const INPROGRESSDATA = res.data.filter(proj => proj.isDeleted == false && proj.isDeleted == false);
                    console.log('VENDOR PROP', props.Vendor[0])
                    console.log("VENDOR CHART DATA", res.data.filter(inv => inv.Vendor == props.Vendor))
                    console.log("VENDOR CHART DATA2", data)
                    setchartData({
                        labels: res.data.filter(inv => inv.Vendor == props.Vendor).map((item) => item.InventoryName),
                        datasets: [
                            {
                                label: "Inventory Amount",
                                data: res.data.filter(inv => inv.Vendor == props.Vendor).map((item) => item.NumofInventory),
    ...
    function goInv() {
        ...
        <Navigate to='bookkeeping/inventory' />
                ...
                return (
                <div style={{textAlign:'-webkit-center'}}className='vendor-inventory-container-chart'>
                    {loading == true ?
                            ...
                            <>
                                {data.length == 0 ?
                                    (
                                        ...
                                        <div className='no-srdata'>
                                            Please Add Inventory Related to the client by clicking <Button className='no-inv-button' href='bookkeeping/inventory'>Go To Inventory</Button>
                                        </div>
                                    )
                                    :
                                    (
                                        <div className='PieChartClientInventory'>
                                            <Bar options={chartOptions} height={60} data={chartData} />
                                        </div>
                                    )
                ...
                export default ClientInventoryChart;