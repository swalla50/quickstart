...
function InProgressProjects(props) {
    ...
    useEffect(() => {
        ...
        setTimeout(() => {
            ...
            axios.get(`${'GetPercentTasks/GetProjPercentages/' + id}`)
                .then((res) => {
                    ...
                    setchartData({
                        ...
                        datasets: [
                            {
                                label: ["Completed Projects", "Uncompleted Projects"],
                                data: [res.data.CompletedProjects, res.data.UncompletedProjects],
    ...
    return (
        <div className='client-inventory-container-chart'>
            {loading == true ?
                ...
                (
                    <>
                        {data.CompletedProjects == 0 || data.length == 0?
                            (

                                <div style={{ width: '100%', height: '20rem' }} className='no-projdata'>
                                   <h7 style={{color:'black'}}>0 Project Progress For This Client</h7><Button className='no-inv-button' href='project'>Add Project</Button>
                                </div>
                            )
                            :
                            (
                                <div style={{ width: '20rem', height: '20rem',color:'black'}} className='PieChartClientProject'>
